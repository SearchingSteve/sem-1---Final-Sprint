...
def move_into_room():
    # if all conditions are true; if only point c has been passed
    if point_c_passed == True and point_e_passed == False and point_g_passed == False:
        print("moving into room 1")
        chassis_ctrl.move_with_distance(0, 2.33)  # Moves into room
        chassis_ctrl.move_with_distance(90,
                                        # Moves right (horizontally) towards marker - BUT, GIMBAL WILL NOT BE AIMING MARKER. WE WILL FIX ELSEWHERE
                                        2.33)
        ...
        chassis_ctrl.move_with_distance(90,
                                        # Moves right (horizontally) towards marker - BUT, GIMBAL WILL NOT BE AIMING MARKER. WE WILL FIX ELSEWHERE
                                        1.94)
        # Moves forward to person scan position
        chassis_ctrl.move_with_distance(0, 4.584)
        time.sleep(5)

    # If point g is passed (we know all prev points have been passed, negate conditions for simplicity)
    elif point_g_passed == True:
...
def move_out_of_room():
    # if all conditions are true; if only point c has been passed
    if point_c_passed and not point_e_passed and not point_g_passed:
        print("moved out of room 1")
        # Moves left (horizontally) towards room exit
        chassis_ctrl.move_with_distance(-90, 2.33)
        # Moves backwards out of room back to room entry point
        chassis_ctrl.move_with_distance(180, 2.33)

    elif point_e_passed and not point_g_passed:  # If point e is passed but not point g
        # Moves backwards to person scan position
        chassis_ctrl.move_with_distance(180, 4.584)
        # Moves left (horizontally) towards room exit
        chassis_ctrl.move_with_distance(-90, 2.33)
...
def vision_recognized_marker_number_one(msg):
    ...
    # Since you found the marker, turn detection off.
    vision_ctrl.disable_detection(rm_define.vision_detection_marker)
    # Aims in attempt to find marker F
...
def vision_recognized_marker_number_two(msg):
    ...
    led_ctrl.set_bottom_led(rm_define.armor_bottom_all,
                            224, 0, 255, rm_define.effect_always_on)
    led_ctrl.set_bottom_led(rm_define.armor_bottom_all,
                            69, 215, 255, rm_define.effect_always_on)
...
def decide_room_action(room_type):
    if room_type == 1:  # If room is a fire room
        # Rotate chassis so the FRONT is facing the door
        chassis_ctrl.rotate_with_degree(rm_define.clockwise, 90)
        time.sleep(0.5)
        # Enter the room
        move_into_room()
        # Move gimbal left to try to find on left side
        gimbal_ctrl.yaw_ctrl(180)
        time.sleep(1)
        ...
        move_out_of_room()
        time.sleep(1)
        # turn chassis -90 degrees to left to recenter
        chassis_ctrl.rotate_with_degree(rm_define.anticlockwise, 90)
...
# Step 14 (Function which is called on step 14): Research something you can do with the robot and implement it here
...
def start():
    ...
    # Step 9: Perform 3 tasks with robot at reset point F
    scan_for_marker()
    ...
    # Step 14: Research something you can do with the robot and implement it here
